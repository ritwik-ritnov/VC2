...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="Not For You, LOL..!!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Joinnn', url='https://t.me/movieZcafe38X')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        await m.reply("<b>Powered By, @movieZcafe38X</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
